  ...
  class RestRCustomField
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SendX::RestRCustomField` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SendX::RestRCustomField`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^custom_field_[a-zA-Z0-9]{22}$/)
    ...
    def valid?
      ...
      return false if !@id.nil? && @id !~ Regexp.new(/^custom_field_[a-zA-Z0-9]{22}$/)
      type_validator = EnumAttributeValidator.new('Integer', [0, 1, 2, 3, 4])

...
module SendX
  class ECustomField
    ...
    # Whether the custom field is shown.
    attr_accessor :shown

    # Whether the custom field is shareable.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SendX::ECustomField` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SendX::ECustomField`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('Integer', [{0=string}, {1=number}, {2=date}, {3=boolean}])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Integer', [{0=string}, {1=number}, {2=date}, {3=boolean}])
    ...
    def hash
      [name, type, shown, is_shareable, description].hash
    end

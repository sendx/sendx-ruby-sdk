...
module SendX
  class ReportApi
    ...
    def get_campaign_report(identifier, opts = {})
      data, _status_code, _headers = get_campaign_report_with_http_info(identifier, opts)
      data
    end
    ...
    # @param identifier [String] Resource identifier with prefix (e.g., &#x60;contact_BnKjkbBBS500CoBCP0oChQ&#x60;)  **Format:** &#x60;&lt;prefix&gt;_&lt;22-character-id&gt;&#x60; 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RestReportData, Integer, Hash)>] RestReportData data, response status code and response headers
    def get_campaign_report_with_http_info(identifier, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ReportApi.get_campaign_report ...'
      end
      # verify the required parameter 'identifier' is set
      if @api_client.config.client_side_validation && identifier.nil?
        fail ArgumentError, "Missing the required parameter 'identifier' when calling ReportApi.get_campaign_report"
      end
      pattern = Regexp.new(/^[a-z_]+_[a-zA-Z0-9]{22}$/)
      if @api_client.config.client_side_validation && identifier !~ pattern
        fail ArgumentError, "invalid value for 'identifier' when calling ReportApi.get_campaign_report, must conform to the pattern #{pattern}."
      end

      # resource path
      local_var_path = '/report/campaign/{identifier}'.sub('{' + 'identifier' + '}', CGI.escape(identifier.to_s))
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'RestReportData'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['TeamApiKey']

      new_options = opts.merge(
        :operation => :"ReportApi.get_campaign_report",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ReportApi#get_campaign_report\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
